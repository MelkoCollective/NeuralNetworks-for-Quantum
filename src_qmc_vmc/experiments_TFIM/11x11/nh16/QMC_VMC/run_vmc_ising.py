...
class VariationalMonteCarlo(tf.keras.Model):
    ...
    def localenergy(self,samples,logpsi):
        ...
        for n in range(len(self.nns)):
            eloc += - self.J * tf.cast((2*samples[:,self.nns[n][0]]-1)*(2*samples[:,self.nns[n][1]]-1), tf.float32)
        ...
        for j in range(self.N):
            ...
            eloc += - self.hx * tf.math.exp(flip_logpsi-logpsi)
    ...
    def buildlattice(self):
        ...
        for x in range(self.Lx):
            for y in range(self.Ly-1):
                self.nns.append([self.coord_to_site(x,y),self.coord_to_site(x,y+1)])
        for y in range(self.Ly):
            for x in range(self.Lx-1):
                self.nns.append([self.coord_to_site(x,y),self.coord_to_site(x+1,y)])
...
def run_VMC(vmc, epochs, hx, qmc_data, energy, variance, batch_size=100):
    '''
    ...
    if qmc_data != None:
        print("Running VMC using QMC data for hx = ", hx)
    else:
        print("Running VMC for hx =",hx)
...
Lx = 11     # Linear size in x direction
Ly = 11     # Linear size in y direction
N = Lx*Ly   # Total number of atoms:w
J = 1.0     # Interaction strength
hx = 3.0    # Transverse field
...
wavefunction = VariationalMonteCarlo(Lx,Ly,J,hx,nh,lr,Lx+Ly,seed)
energy = []
variance = []
...
if qmc_epochs != 0:
    ...
    path = "../../../../../QMC_data"
    dim_path = "TFIM_Dim={}_M=1000000_J={}_hx={}".format(Lx, J, hx) # Can change this to look at Dim = 4, 8, 12, 16
    ...
    wavefunction, energy, variance = run_VMC(wavefunction, qmc_epochs, hx, qmc_dataset, energy, variance, batch_size)
...
if vmc_epochs != 0:
    wavefunction, energy, variance = run_VMC(wavefunction, vmc_epochs, hx, None, energy, variance, batch_size)
    
a_file = open("E_TFIM_QMC_VMC_{}_{}.dat".format(Lx, nh), "w")
np.savetxt(a_file, energy)
a_file.close()
